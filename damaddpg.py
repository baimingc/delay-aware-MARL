...
#     maddpg = MADDPG.init_runner_from_save(model_path)
    ...
    maddpg = MADDPG.init_from_env_with_runner(env, agent_alg=config.agent_alg,
...
#                                   for acsp in env.action_space[:2]])
    replay_buffer = ReplayBuffer(config.buffer_length, maddpg.nagents,
                                 [env.observation_space[0].shape[0] + delay_step*2, env.observation_space[1].shape[0] + delay_step*2, env.observation_space[-1].shape[0] ],
#                                  [18, 18, 12],
                                 [acsp.shape[0] if isinstance(acsp, Box) else acsp.n
...
#         print(last_agent_actions)
        for a_i, agent_obs in enumerate(obs[0][:2]):
...
#             next_obs, rewards, dones, infos = env.step(copy.deepcopy(actions))
# #             print('6', actions)
            for a_i, agent_obs in enumerate(next_obs[0][:2]):
                for _ in range(len(last_agent_actions)):
